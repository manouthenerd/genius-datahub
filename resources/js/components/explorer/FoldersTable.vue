<template>
    <table class="w-full text-left text-sm text-gray-500 rtl:text-right dark:text-gray-400">
            ...
            <tr>
                ...
                <th scope="col" class="px-6 py-3">
                    <Popover>
                        ...
                        <PopoverContent>
                            <div class="my-2">
                                <p class="text-xs">Créer un nouveau dossier</p>
                            </div>
                            <Form
                                method="POST"
                            ...
                            >
                                ...
                                <div>
                                    <Button :disabled="processing">
                                        <Loader2Icon class="animate-spin" v-if="processing" />
                                        Enregistrer
                                    </Button>
                                </div>
                            </Form>
        ...
        <tbody v-if="folders.length">
            ...
            <tr v-for="folder in folders" :key="folder.id" class="border-y border-gray-200 transition-all dark:border-gray-700 dark:bg-gray-800">
                <th scope="row" class="cursor-pointer px-6 py-2 font-medium whitespace-nowrap text-gray-900 dark:text-white">
                    <ContextMenu>
                        <ContextMenuTrigger>
                            <Link :href="route('resources.files', { service: service.id, folder: folder.id })" class="flex items-center gap-1">
                                <Folder :size="16" fill="#FFD700" /> <span>{{ folder.name }}</span>
                            </Link>
                        </ContextMenuTrigger>
                        <ContextMenuContent>
                            <ContextMenuSub>
                                ...
                                <ContextMenuSubContent v-if="showContext">
                                    <Form
                                        ...
                                        class="grid gap-2"
                                        method="PUT"
                                        :action="route('folders.update', { folder: Number(folder.id) })"
                                        v-slot="{ errors, processing }"
                                    >
                                        <Input
                                            type="text"
                            ...
                            <ContextMenuItem>
                                <Form method="DELETE" :action="route('folders.destroy', { folder: folder.id })">
                                    <Button style="color: red" variant="link"> <Trash color="red" />Supprimer </Button>
                                </Form>
                            </ContextMenuItem>
                        </ContextMenuContent>
                    </ContextMenu>
                </th>
                <td class="px-6 py-2">{{ folder.size }} Mo</td>
                <td class="px-6 py-2">
                    {{ new Date(folder.updated_at).toLocaleString('GB') }}
                </td>
            </tr>
        </tbody>
        ...
        <tbody v-else class="w-full space-y-2">
            ...
            <tr>
                <th colspan="5" class="text-center">
                    <p class="mt-4 w-full p-2 text-center text-sm text-black underline">Aucun dossier trouvé.</p>
                </th>
            </tr>
        </tbody>
    </table>
...
import { Ellipsis, Folder, Loader2Icon, Trash } from 'lucide-vue-next';
import { ref } from 'vue';
...
defineProps<{ folders: Folder[]; service: Service }>();
